...
def save_images(epoch):
    ...
    fake = netG(conditional(x), fixed_z)
    ...
    for i in range(len(dataloader['train'])):
        ...
        for e_iter in range(updates['e']['num_updates']):
            ...
            for dd in range(4):
                ...
                # g(Z)
                fake = netG(cc[dd:dd + 1].expand_as(cc), z).detach()
                # e(g(Z))
                egz = netE(fake)
        ...
        if opt.adaptive_advesary_weight:
            ...
            if stats['KL_real'] > ideal_KL * 1.05 or stats['KL_fake'] > ideal_KL * 1.05:
